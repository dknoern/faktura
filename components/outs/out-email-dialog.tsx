...
interface OutEmailDialogProps {
  open: boolean;
  onOpenChange: (open: boolean) => void;
  outId: string;
}

export function OutEmailDialog({ open, onOpenChange, outId }: OutEmailDialogProps) {
      ...
      <DialogContent className="sm:max-w-[500px]">
        <DialogHeader>
          <DialogTitle className="flex items-center gap-2">
            ...
            Send Log Out Entry via Email
          </DialogTitle>
          <DialogDescription>
            Enter one or more email addresses (comma-separated) to send this log out entry to.
          </DialogDescription>
        </DialogHeader>
        
        <div className="grid gap-4 py-4">
          <div className="grid gap-2">
            <Label htmlFor="out-email-addresses">
              Email Addresses <span className="text-red-500">*</span>
            </Label>
            <Input
              id="out-email-addresses"
